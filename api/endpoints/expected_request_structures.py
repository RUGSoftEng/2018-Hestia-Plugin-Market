...
class SortOptionSchema(Schema):
    ...
    by = fields.String(load_only=True, required=True)
    #  The type of ordering.
    order = fields.String(load_only=True, required=True)
...
class PluginOptionsSchema(Schema):
    ...
    count = fields.Integer(load_only=True, required=False)
    #  An optional author id to filter by.
    author_id = fields.String(load_only=True, required=False)
    #  A substring used in searching for matching titles.
    title_contains = fields.String(load_only=True, required=False)
    #  A list of tag names to filter by.
    tags = fields.List(fields.String(), load_only=True, required=False)
    #  A sorting object which contains two values: by, and order.
    sort = fields.Nested(SortOptionSchema, load_only=True, required=False)
